...
class MaterialEditor:
    ...
    def __init__(self, obj=None, prop=None, material=None, card_path="", category="Solid"):
        ...
        if self.card_path:
            ...
            normalized_path = os.path.normpath(self.card_path)
            self.initialIndex = self.widget.ComboMaterial.findData(normalized_path)
            
            # If not found, try with forward slashes
            if self.initialIndex == -1:
                forward_path = normalized_path.replace('\\', '/')
                self.initialIndex = self.widget.ComboMaterial.findData(forward_path)
            
            # If still not found, try matching by filename only
            if self.initialIndex == -1:
                filename = os.path.basename(normalized_path)
                for i in range(self.widget.ComboMaterial.count()):
                    item_path = self.widget.ComboMaterial.itemData(i)
                    if item_path and os.path.basename(item_path) == filename:
                        self.initialIndex = i
                        break
            
            FreeCAD.Console.PrintMessage("Initial card path: {}, found index: {}\n".format(self.card_path, self.initialIndex))
    ...
    def chooseMaterial(self, index):
        ...
        # Handle empty or None card paths (e.g., the blank entry at index 0)
        if not self.card_path:
            FreeCAD.Console.PrintMessage("No material card selected (blank entry)\n")
            return
        ...
        elif os.path.isdir(self.card_path):
            FreeCAD.Console.PrintError("Material card path is a directory, not a file: {}\n".format(self.card_path))
        else:
    ...
    def updateCardsInCombo(self):
        ...
        def find_materials_base_directory():
            ...
            for path in self.cards.keys():
                if os.path.isdir(path):
                    # Walk up to find the Materials directory
                    parts = path.split(os.sep)
                    for i, part in enumerate(parts):
                        if part == 'Materials':
                            return os.sep.join(parts[:i+1])
                    # If not found, use the parent of the path
                    return os.path.dirname(path)
            return None

        def collect_all_fcmat_files(base_dir):
            """Recursively collect all .FCMat files from the materials directory."""
            all_cards = []
            seen_names = set()
            ...
            for root, dirs, files in os.walk(base_dir):
                for f in sorted(files):
                    if f.endswith('.FCMat'):
                        fcmat_path = os.path.join(root, f)
                        card_name = f[:-6]  # Remove .FCMat extension
                        
                        # Avoid duplicates
                        if card_name not in seen_names:
                            seen_names.add(card_name)
                            all_cards.append([card_name, fcmat_path, ""])
        ...
        # Update self.cards and self.materials to include all found materials
        # This ensures that code that relies on these dictionaries works correctly
        for card_name, card_path, icon_path in card_name_list:
            self.cards[card_path] = card_name
            # Try to read density from the file
            try:
                from importFCMat import read
                mat_data = read(card_path)
                self.materials[card_path] = mat_data
            except Exception:
    ...
    def openfile(self):
        ...
        "Opens a FCMat file"
        # Find the correct materials directory
        # First try the system resources directory
        resource_dir = FreeCAD.getResourceDir()
        materials_dir = os.path.join(resource_dir, "Mod", "Material", "Resources", "Materials")
        
        # If not found, try the user directory
        if not os.path.isdir(materials_dir):
            user_dir = self.directory
            if user_dir:
                # Try various possible paths
                possible_paths = [
                    os.path.join(user_dir, "Resources", "Materials"),
                    os.path.join(user_dir, "Materials"),
                    user_dir
                ]
        ...
        if self.category == "Solid":
            # Search for Standard directory in materials
            standard_dir = os.path.join(materials_dir, "Standard")
            if os.path.isdir(standard_dir):
                initial_path = standard_dir
            else:
                initial_path = materials_dir
        else:
            initial_path = materials_dir
        
        # Ensure we start with a valid directory path
        if not os.path.isdir(initial_path):
            initial_path = materials_dir if os.path.isdir(materials_dir) else self.directory
        
        FreeCAD.Console.PrintMessage("Opening file dialog from: {}\n".format(initial_path))
        ...
        if os.path.isfile(filetuple[0]):
            card_path_new = os.path.normpath(filetuple[0])  # Normalize the path
        else:
            FreeCAD.Console.PrintError("Error: Invalid path to the material file\n")
            return
        
        # Try to find the card in the combo box using normalized path
        index = self.widget.ComboMaterial.findData(card_path_new)
        
        # If not found, try with forward slashes (for cross-platform compatibility)
        if index == -1:
            card_path_forward = card_path_new.replace('\\', '/')
            index = self.widget.ComboMaterial.findData(card_path_forward)
        
        # If still not found, try comparing by filename only (in case paths differ but files are the same)
        if index == -1:
            filename_new = os.path.basename(card_path_new)
            for i in range(self.widget.ComboMaterial.count()):
                card_path = self.widget.ComboMaterial.itemData(i)
                if card_path and os.path.basename(card_path) == filename_new:
                    index = i
                    card_path_new = card_path  # Use the known path from combo box
                    break
        ...
        if index == -1:
            FreeCAD.Console.PrintMessage(
                ...
                "The material parameters only are loaded.\n"
                .format(card_path_new)
            )
